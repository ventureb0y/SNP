...
const ProductPage = ({ res }) => {
    ...
    const { id } = data
    const { name, price, old_price, description, dies, images, characteristics } = data.attributes
    const chars = []
    for (var char in characteristics) {
        chars.push({char: char, value: characteristics[char]})
    }
                        ...
                        <div className={styles.product__info}>
                            ...
                            <div className={styles.product__info_chars}>
                                <ul className={styles.product__info_char}>
                                    {chars.map((c => 
                                    <li key={c.char} className={styles.product__info_char_line}>{c.char}:</li>
                                    ))}
                                </ul>
                                <ul className={styles.product__info_char}>
                                {chars.map((c => 
                                    <li key={c.char} className={styles.product__info_char_line}>{c.value}</li>
                                    ))}
                                </ul>
                            </div>
